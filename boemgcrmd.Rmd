...
df <- df %>%
  mutate(
    ...
    prey_category_clean = case_when(
      ...
      prey_category_clean %in% c("mollusc", "molusca", "squid (mollusca)", "squid")                ~ "mollusca",
      ...
      prey_category_clean == "unidentifiable fish?"                                        ~ "biomatter",
       prey_category_clean == "unidentifiable fish"                                        ~ "biomatter",
      prey_category_clean == "striped anchovy"                                             ~ "anchovy",
       prey_category_clean == "thread herring"                                             ~ "thread herring",
       prey_category_clean == "cnidarian"                                             ~ "cnidarian",
      prey_category_clean == "veg_biomatter" ~ "veg_biomatter",
      prey_category_clean %in% "synthetic material" ~ "synthetic material",
...
```{r}
# ============================================================
# 5.5 CLEANING n_prey COLUMN
#
# Contains mixed values:
#   "-", "", " " = not recorded --> NA
#   "1?", "6?"   = uncertain counts --> take the number, flag uncertainty
#   "~4"         = approximate --> take the number
#   ">20", ">50" = minimum count --> use value as-is (conservative)
#   "<20", "<50" = maximum count --> use value as-is (conservative)
#   ">1"         = use 1
#   "20+", ">20" = same thing --> use value
#   "?"          = unknown --> NA
#   numbers      = keep as-is
#
# n_prey_flag:
#   "exact"       = recorded as a plain number
#   "approximate" = recorded with ~ or ?
#   "minimum"     = recorded as > or +  (true count is AT LEAST this)
#   "maximum"     = recorded as <       (true count is AT MOST this)
#   NA            = missing entirely
# ============================================================
df_clean <- df_clean %>%
  mutate(
    n_prey_raw = str_trim(as.character(n_prey)),
    
    n_prey_flag = case_when(
      n_prey_raw %in% c("-", "", " ", "?")       ~ NA_character_,
      str_detect(n_prey_raw, "^[0-9]+\\?$")       ~ "approximate",
      str_detect(n_prey_raw, "^~")                ~ "approximate",
      str_detect(n_prey_raw, "^>")                ~ "minimum",
      str_detect(n_prey_raw, "^[0-9]+\\+$")       ~ "minimum",
      str_detect(n_prey_raw, "^<")                ~ "maximum",
      TRUE                                         ~ "exact"
    ),
    
    n_prey_clean = case_when(
      n_prey_raw %in% c("-", "", " ", "?")        ~ NA_real_,
      str_detect(n_prey_raw, "^[0-9]+\\?$")        ~ as.numeric(str_remove(n_prey_raw, "\\?")),
      str_detect(n_prey_raw, "^~")                 ~ as.numeric(str_remove(n_prey_raw, "~")),
      str_detect(n_prey_raw, "^>")                 ~ as.numeric(str_remove(n_prey_raw, ">")),
      str_detect(n_prey_raw, "^[0-9]+\\+$")        ~ as.numeric(str_remove(n_prey_raw, "\\+")),
      str_detect(n_prey_raw, "^<")                 ~ as.numeric(str_remove(n_prey_raw, "<")),
      TRUE                                          ~ suppressWarnings(as.numeric(n_prey_raw))
    )
  ) %>%
  select(-n_prey_raw)

# Sanity check - verify every raw value maps to expected flag + number
df_clean %>% 
  count(n_prey, n_prey_clean, n_prey_flag, sort = TRUE)
...
df_clean <- df_clean %>%
  mutate(
    ...
    collection_date = case_when(
      as.integer(format(collection_date, "%Y")) > 2026 ~ collection_date - lubridate::years(70),
      TRUE ~ collection_date
    )
  )
# How many stomachs had at least one identifiable prey per species
identifiable_summary <- df_clean %>%
  filter(!prey_category_clean %in% unidentifiable) %>%
  group_by(fish_species) %>%
  summarise(
    n_stomachs_total      = n_distinct(fish_id),
    n_with_identifiable   = n_distinct(fish_id[!is.na(prey_category_clean)]),
    pct_with_identifiable = round((n_with_identifiable / n_stomachs_total) * 100, 1)
  ) %>%
  arrange(desc(pct_with_identifiable))

print(identifiable_summary)

# Breakdown of identifiable prey categories per species
identifiable_breakdown <- df_clean %>%
  filter(!prey_category_clean %in% unidentifiable) %>%
  group_by(fish_species, prey_category_clean) %>%
  summarise(
    n_stomachs_with_prey = n_distinct(fish_id),
    .groups = "drop"
  ) %>%
  left_join(n_stomachs, by = "fish_species") %>%
  mutate(pct = round((n_stomachs_with_prey / n_stomachs_total) * 100, 1)) %>%
  arrange(fish_species, desc(pct))

print(identifiable_breakdown, n = 60)
...
prey_colors <- c(
  ...
  "crustacean"          = "#2196F3",
  "anchovy"             = "#FF5722",
  "veg_biomatter"       = "#FF9800",
  "polychaete"          = "#9C27B0",
  "mollusca"            = "#4CAF50",
  "detritus"            = "#795548",
  "echinoderm"          = "#E91E63",
  "synthetic material"  = "#FF0000",
  "thread herring"      = "#FFFFE0",
  "cnidarian"           = "#CBC3E3"
...
```{r}
# ============================================================
# PREDATOR-PREY NETWORK GRAPH
# ============================================================
# manually set node positions so labels have room
predator_names <- unique(edges$from)
prey_names     <- unique(edges$to)

node_positions <- bind_rows(
  tibble(
    name = predator_names,
    x    = 0,
    y    = seq(0, 1, length.out = length(predator_names))
  ),
  tibble(
    name = prey_names,
    x    = 1,
    y    = seq(0, 1, length.out = length(prey_names))
  )
)

#creating nodes
nodes <- node_positions %>%
  mutate(type = name %in% predator_names)
#create graph
net <- tbl_graph(nodes = nodes, edges = edges, directed = TRUE)

# use manual layout
layout_manual <- create_layout(net, layout = "manual",
                                x = nodes$x, y = nodes$y)

p_network <- ggraph(layout_manual) +
  geom_edge_link( ##creating lines
    aes(width = weight, alpha = weight, color = prey_label),
    lineend = "round"
  ) + #scaling lines according to FOO
  scale_edge_width(range = c(0.3, 3), name = "FOO (%)") +
  scale_edge_alpha(range = c(0.2, 0.9), guide = "none") +
  scale_edge_color_manual(values = prey_colors, name = "Prey category") +
  geom_node_point(aes(shape = type), size = 4, color = "#333333") +
  geom_node_text( #creating nodes in graph
    aes(
      label = name,
      x     = x + ifelse(type == TRUE, -0.03, 0.03),
      hjust = ifelse(type == TRUE, 1, 0)
    ),
    size  = 3.5,
    color = "#333333"
  ) +
  scale_shape_manual(
    ...
    labels = c("TRUE" = "Predator", "FALSE" = "Prey"),
    name   = "Node type"
  ) +
  labs(
    title    = "Predator–Prey Diet Network",
    subtitle = "Edge width proportional to Frequency of Occurrence (%)\nIdentifiable prey only"
  ) +
  ...
  xlim(-0.4, 1.4)  # extra horizontal padding for labels

p_network
ggsave("predator_prey_network.png", p_network, width = 12, height = 9, dpi = 300)
```
##another kind of graph 
```{r}
# build color matrix explicitly because the other one isnt working grrrrr
circos.clear()
circos.par(
  gap.after    = c(rep(3, nrow(chord_data) - 1), 15,
                   rep(3, ncol(chord_data) - 1), 15),
  start.degree = 150,
  canvas.xlim  = c(-1.3, 1.3),  # more canvas room for labels
...
# Build prey matrix - Lizardfish excluded as outlier
prey_matrix <- df_clean %>%
  filter(!is.na(prey_category_clean)) %>%
  mutate(present = 1) %>%
  distinct(fish_id, fish_species, region, collection_date, position, prey_category_clean, .keep_all = TRUE) %>%
  pivot_wider(
    id_cols     = c(fish_id, fish_species, region, collection_date, position),  # <-- add collection_date
...
# PERMANOVA: does diet differ by region?
adonis2(prey_cols ~ region, data = prey_matrix %>% filter(!is.na(region)), permutations = 999)

# PERMANOVA: do fish caught on the same day have similar diets?
adonis2(prey_cols ~ collection_date, data = prey_matrix %>% filter(!is.na(collection_date)), permutations = 999)

#combined 
adonis2(prey_cols ~ fish_species + collection_date, 
        data = prey_matrix %>% filter(!is.na(collection_date)), 
        permutations = 999)

# PERMANOVA: does diet differ by inner vs outer shoal position?
adonis2(prey_cols ~ position, 
        data = prey_matrix %>% filter(!is.na(position)), 
        permutations = 999)

# Combined: species + position
adonis2(prey_cols ~ fish_species + position, 
        data = prey_matrix %>% filter(!is.na(position)), 
        permutations = 999)

# Full model: species + region + position
adonis2(prey_cols ~ fish_species + region + position, 
        data = prey_matrix %>% filter(!is.na(position) & !is.na(region)), 
        permutations = 999)
```
**Interpertation**
Diet composition differed significantly by fish species (PERMANOVA, F = 14.5, R² = 0.11, p = 0.001) and weakly by region (F = 2.84, R² = 0.003, p = 0.036), but not by shoal position (F = 1.96, p = 0.109) or collection date (F = 1.16, p = 0.314), suggesting that species identity is the primary driver of diet variation across the sampling area.
